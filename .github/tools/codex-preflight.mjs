...
const sh = (c, opt = {}) => {
  try { return execSync(c, { stdio: "pipe", encoding: "utf8", ...opt }); }
  catch (e) { if (!opt.ignore) throw e; return e.stdout?.toString() ?? ""; }
};

const base = process.env.CI_BASE || "main";
const top = sh("git rev-parse --show-toplevel").trim();
if (!top) { console.error("no repo"); process.exit(1); }
process.chdir(top);
...
try { sh("npm test --silent", { ignore: true }); } catch {}
